...
"""
    get_fallback(model::MOI.ModelLike, ::MOI.ObjectiveBound, T::Type)::T

Compute the objective bound value of type `T` using the `ConstraintDual` results
and the `ConstraintFunction` and `ConstraintSet` values.
"""
function get_fallback(model::MOI.ModelLike, ::MOI.ObjectiveBound, T::Type)
